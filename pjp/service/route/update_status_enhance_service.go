...
func (service *routeService) UpdateStatusEnhance(ctx context.Context, request request.UpdateStatusEnhanceRequest, custId string) {
	...
	for i := range request.PjpCode {
		...
		route := service.routeOutletRepo.FindAllOutletsByPjpCode(ctx, tx, pjpCode, custId)
		...
		if request.Status == "Approved" {
			...
			if len(routes) != 0 {
				for _, route := range routes {
					if !shouldCreateDailyRoute(route, now) {
						continue
					}
					dataset := model.RoutePopDaily{
						...
						Day:                  route.Day,
						Date:                 route.Date,
						WorkingDayCalendarID: route.WorkingDayCalendarID,
		...
		for _, data := range route {
			routes := model.RouteOutlet{
				...
				VerifiedDate: &now,
				// RouteCode:    route.RouteCode,
				// OutletCode:   request.OutletCode[i],
			}
			service.routeOutletRepo.UpdatePivot(ctx, tx, routes)
		}
	}
}

func shouldCreateDailyRoute(route model.RoutePopPermanent, now time.Time) bool {
	return !helper.IsBeforeCurrentWeek(route.Date, now)
}
