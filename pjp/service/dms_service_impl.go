...
// 	// find outletId by pjpIds
// 	outletIds := service.routeOutletRepo.FindAllOutletIdByPjpIdToday(ctx, pjpIds)
// 	if len(outletIds) == 0 {
...
// 	}
// 	log.Printf("Retreived outletIDs: %d", outletIds)

// 	var outletIdStrings []string
// 	for _, outletId := range outletIds {
// 		outletIdStrings = append(outletIdStrings, strconv.Itoa(outletId))
// 	}
// 	outletIdsFormatted := strings.Join(outletIdStrings, ",")

// 	// remove /master when deploy to staging or production
// 	endpointURL := fmt.Sprintf("%s/v1/outlets?&outlet_id=%s&limit=99", config.KongUrl, outletIdsFormatted)
...
// 	// Determine outletIds based on dataFilter.Search
// 	var outletIds []int
// 	if dataFilter.Search != "" {
// 		log.Printf("Search filter applied: %s", dataFilter.Search)
// 		outletIds = service.routeOutletRepo.SearchOutletIdByPjpId(ctx, pjpIds, dataFilter.Search)
// 	} else {
// 		log.Printf("No search filter applied")
// 		outletIds = service.routeOutletRepo.FindAllOutletIdByPjpId(ctx, pjpIds)
// 	}

// 	// Handle empty outletIds
// 	if len(outletIds) == 0 {
// 		emptyPagination := response.Meta{
// 			TotalData: 0,
// 			Page:      0,
...
// 		return []model.OutletNew{}, emptyPagination, nil
// 	}
// 	log.Printf("Retrieved outletIDs: %d", outletIds)

// 	// Convert outletIds to comma-separated string
// 	var outletIdStrings []string
// 	for _, outletId := range outletIds {
// 		outletIdStrings = append(outletIdStrings, strconv.Itoa(outletId))
// 	}
...
// 	// Remove /master when deploy to staging or production
// 	endpointURL := fmt.Sprintf("%s/v1/outlets?&outlet_id=%s", config.KongUrl, outletIdsFormatted)
// 	log.Printf("Request URL: %s", endpointURL)
