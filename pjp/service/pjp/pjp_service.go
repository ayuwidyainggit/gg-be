...
type PjpService interface {
	...
	GetDestinationDetails(ctx context.Context, request request.DestinationDetailsRequest, currentCustomerId string) (response.DestinationDetailsData, response.LiveMonitoringPaging, error)
}
...
func NewPjpService(pjpRepo pjp.PjpRepository, routeOutletRepository routeoutlet.RouteOutletRepository, routeRepository route.RouteRepository, validate *validator.Validate, db *gorm.DB) PjpService {
	return &pjpService{
