...
func (service *RoutePopServiceImpl) FindAllDaily(ctx context.Context, filters map[string]interface{}, currentCustomerId string) []response.RoutePopDailyResponse {
	...
	for _, value := range result {
		...
		if value.Route != nil {
			for _, routeOutlet := range value.Route.RouteOutlets {
				outlet := response.OutletMap{
					OutletID:      routeOutlet.OutletID,
					OutletCode:    routeOutlet.OutletCode,
					OutletName:    routeOutlet.OutletName,
					Longitude:     routeOutlet.Longitude,
					Latitude:      routeOutlet.Latitude,
					OutletStatus:  routeOutlet.OutletStatus,
					OutletAddress: routeOutlet.OutletAddress,
...
func (service *RoutePopServiceImpl) FindByRouteOutletAdditional(ctx context.Context, code int, currentCustomerId string) response.RouteDetailResponse {
	...
	var outletResponses []response.OutletResponse
	if len(data) > 0 {
		for _, value := range data {
			for _, routeOutlet := range value.Route.RouteOutlets {
				...
				outletResponses = append(outletResponses, outlet)
			}
		}
	}
	...
	response.Outlets = outletResponses
	return response
