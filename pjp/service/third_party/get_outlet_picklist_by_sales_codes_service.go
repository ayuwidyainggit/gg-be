...
func (service *thirdPartyService) GetOutletPicklistBySalesCodes(ctx context.Context, dataFilter model.OutletBySalesman, headers map[string]string, custId string) ([]model.OutletNew, response.Meta, error) {
	...
	outlets := service.routeOutletRepo.FindAllOutletsByPjpIds(ctx, tx, pjpIds, custId)
	if len(outlets) == 0 {
