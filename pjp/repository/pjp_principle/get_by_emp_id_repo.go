...
func (repo *pjpPrincipleRepository) GetPjpsByEmpId(ctx context.Context, tx *gorm.DB, empID int, currentCustomerId string) model.PjpPrinciple {
	var pjps model.PjpPrinciple

	err := tx.WithContext(ctx).
		Select("pjp_principles.permanent_journey_plans.*, cust.distributor_id").
		Where("pjp_principles.permanent_journey_plans.cust_id = ? AND pjp_principles.permanent_journey_plans.salesman_id = ?", currentCustomerId, empID).
