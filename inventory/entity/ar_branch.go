...
const (
	AR_BRANCH_INVOICE_STATUS_OUTSTANDING = 1
	AR_BRANCH_INVOICE_STATUS_PAID        = 2
)

var arBranchInvoiceStatusNameList = map[int64]string{
	AR_BRANCH_INVOICE_STATUS_OUTSTANDING: "Outstanding",
	AR_BRANCH_INVOICE_STATUS_PAID:        "Paid",
}

const (
	AR_BRANCH_PAYMENT_OPTION_PARTIAL = 1
	AR_BRANCH_PAYMENT_OPTION_FULL    = 2
)

var arBranchPaymentOptionNameList = map[int64]string{
	AR_BRANCH_PAYMENT_OPTION_PARTIAL: "Partial",
	AR_BRANCH_PAYMENT_OPTION_FULL:    "Full",
}

const (
	AR_BRANCH_PAYMENT_TYPE_CASH         = 1
	AR_BRANCH_PAYMENT_TYPE_CHEQUE       = 2
	AR_BRANCH_PAYMENT_TYPE_TRANSFER     = 3
	AR_BRANCH_PAYMENT_TYPE_RETURN       = 4
	AR_BRANCH_PAYMENT_TYPE_DEBIT_CREDIT = 5
)

var arBranchPaymentTypeNameList = map[int64]string{
	AR_BRANCH_PAYMENT_TYPE_CASH:         "Cash",
	AR_BRANCH_PAYMENT_TYPE_CHEQUE:       "Cheque",
	AR_BRANCH_PAYMENT_TYPE_TRANSFER:     "Transfer",
	AR_BRANCH_PAYMENT_TYPE_RETURN:       "Return",
	AR_BRANCH_PAYMENT_TYPE_DEBIT_CREDIT: "Debit/Credit",
}
...
type ArBranchQueryFilter struct {
	ParentCustId string
...
type ArBranchDetailQuery struct {
	ArBranchCustId string `query:"cust_id" validate:"required"`
}

type ArBranchDistributorsFilterQueryFilter struct {
...
type ArBranchSuppliersFilterQueryFilter struct {
	// StartDate *int64 `query:"startDate" validate:"required_with=EndDate,omitempty,gte=1000000000"`
	// EndDate   *int64 `query:"endDate" validate:"required_with=StartDate,omitempty,lte=9999999999,gtefield=StartDate"`
	Page  int    `query:"page"`
	Limit int    `query:"limit" validate:"required"`
...
*/
type ArBranchListResponse struct {
	CustId            string   `json:"cust_id"`
	...
	ParentCustName    *string  `json:"parent_cust_name"`
...
*/
type ArBranchDistributorsFilterListResponse struct {
	CustID   string `json:"cust_id"`
	CustName string `json:"cust_name"`
}

type ArBranchSuppliersFilterListResponse struct {
	SupID   int64  `json:"sup_id"`
	SupCode string `json:"sup_code"`
	SupName string `json:"sup_name"`
}

type DetailArBranchParams struct {
	GrBranchNo string `params:"gr_branch_no" validate:"required"`
}

type CreateArBranchPaymentParams struct {
	GrBranchNo string `params:"gr_branch_no" validate:"required"`
}
...
*/
type ArBranchWithDetailResponse struct {
	CustId       string   `json:"cust_id"`
	CustName     string   `json:"cust_name"`
	GrBranchNo   string   `json:"gr_branch_no"`
	GrBranchDate string   `json:"gr_branch_date"`
	...
	PaidAmount           *float64                  `json:"paid_amount"`
	RemainingAmount      *float64                  `json:"remaining_amount"`
	Details              ArBranchDetListGroup      `json:"details"`
	Payments             []ArBranchPaymentResponse `json:"payments"`
}
...
*/
func (grBranch ArBranchWithDetailResponse) GenerateDataStatusName() string {
	return grBranchDataStatusNameList[grBranch.DataStatus]
}

func (orderBooking ArBranchWithDetailResponse) GenerateOrderBookingTypeApprovalName() string {
...
func (grBranch ArBranchListResponse) GenerateDataStatusName() string {
	return grBranchDataStatusNameList[grBranch.DataStatus]
}

func (arBranch ArBranchWithDetailResponse) GenerateInvoiceStatusName() string {
	if arBranch.InvoiceStatus == nil {
		return ""
	}

	return arBranchInvoiceStatusNameList[*arBranch.InvoiceStatus]
}

func (arBranch ArBranchListResponse) GenerateInvoiceStatusName() string {
	if arBranch.InvoiceStatus == nil {
		return ""
	}
...
func (payment ArBranchPaymentResponse) GeneratePaymentOptionName() string {
	return arBranchPaymentOptionNameList[int64(payment.PaymentOption)]
}
func (payment ArBranchPaymentResponse) GeneratePaymentTypeName() string {
	return arBranchPaymentTypeNameList[int64(payment.PaymentType)]
}
func (payment ArBranchPaymentResponse) GenerateVerificationStatusName() string {
	return arBranchVerificationStatusNameList[int64(payment.VerificationStatus)]
}
