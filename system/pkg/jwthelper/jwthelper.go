...
func GenerateNewToken(user entity.UserData, credentials []string, additionalParam map[string]interface{}) (*entity.Token, error) {
...
func ExtractTokenMetadata(c *fiber.Ctx) (*entity.TokenMetadata, error) {
	...
	if ok && token.Valid {
		...
		distPriceGrpId := claims["dist_price_grp_id"].(float64)
		...
		return &entity.TokenMetadata{
			...
			DistPriceGrpId: int(distPriceGrpId),
		}, nil
	}
...
func generateNewAccessToken(user entity.UserData, credentials []string, additionalParam map[string]interface{}) (string, error) {
	...
	claims["parent_cust_id"] = user.ParentCustId
	claims["dist_price_grp_id"] = user.DistPriceGrpId
	claims["emp_id"] = user.EmpId
	claims["is_admin"] = user.IsAdmin
	claims["distributor_id"] = user.DistributorID
