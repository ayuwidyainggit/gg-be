...
func (repo *RoutePopPermanentRepositoryImpl) FindAll(ctx context.Context, filters map[string]interface{}, currentCustomerId string) []model.RoutePopPermanent {
	...
	for field, value := range filters {
		...
		case "salesman_name":
			if v, ok := value.(string); ok && v != "" {
				query = query.Joins("JOIN pjp_principles.permanent_journey_plans AS pjp_plans ON pjp_principles.route_pop_permanent.pjp_id = pjp_plans.id").
	...
	// query = query.Joins("JOIN pjp_principles.routes ON pjp_principles.route_pop_permanent.route_code = pjp_principles.routes.route_code").
	// 	Select("pjp_principles.route_pop_permanent.*, pjp_principles.routes.route_name")
...
func (repo *RoutePopPermanentRepositoryImpl) GetAllVisitDayMap(ctx context.Context, dataFilter entity.VisitDayMapQueryFilter, currentCustomerId string) []model.VisitDayMap {
	...
	query := repo.Db.Table("pjp_principles.route_pop_permanent").
		Select("pjp_principles.route_pop_permanent.id, pjp_principles.destinations.route_code,  pjp_principles.destinations.route_name, pjp_principles.route_pop_permanent.date, pjp_principles.route_pop_permanent.week, COUNT(pjp_principles.destinations.outlet_id) AS total_outlet").
		Joins("LEFT JOIN pjp_principles.destinations ON pjp_principles.route_pop_permanent.route_code = pjp_principles.destinations.route_code").
		Group("pjp_principles.route_pop_permanent.id, pjp_principles.destinations.route_code, pjp_principles.destinations.route_name, pjp_principles.route_pop_permanent.date, pjp_principles.route_pop_permanent.week")

	if dataFilter.PjpCode != 0 {
		query = query.Where("pjp_principles.route_pop_permanent.pjp_code = ?", dataFilter.PjpCode)
	}
	...
	if dataFilter.Sort != "" {
		...
		query = query.Order("pjp_principles.route_pop_permanent.id DESC")
	}
...
// 	repo.Db.Table("pjp_principles.destinations").
// 		Select("pjp_principles.destinations.route_code, COUNT(*) as total_outlet").
// 		Joins("JOIN pjp_principles.routes ON pjp_principles.destinations.route_code = pjp_principles.routes.route_code").
// 		Where("cust_id = ?", currentCustomerId).
// 		Group("pjp_principles.destinations.route_code, pjp_principles.routes.route_name").
// 		Scan(&results)
...
func (repo *RoutePopPermanentRepositoryImpl) CountOutletByRoute(ctx context.Context, currentCustomerId, startDate, endDate string) map[int]struct {
	...
	mainQuery := `
		SELECT pjp_principles.destinations.route_code, COUNT(*) AS total_outlet, pjp_principles.routes.route_name
		FROM pjp_principles.destinations
		JOIN pjp_principles.routes ON pjp_principles.destinations.route_code = pjp_principles.routes.route_code
		WHERE pjp_principles.destinations.cust_id = ? AND (pjp_principles.destinations.status = 'Approved' OR pjp_principles.destinations.status = 'Approved With Propose')
		GROUP BY pjp_principles.destinations.route_code, pjp_principles.routes.route_name
	`

	// destinations_additional --> additional
	additionalQuery := `
		SELECT pjp_principles.destinations_additional.route_code, COUNT(*) AS total_outlet, pjp_principles.routes.route_name
		FROM pjp_principles.destinations_additional
		JOIN pjp_principles.routes ON pjp_principles.destinations_additional.route_code = pjp_principles.routes.route_code
		JOIN pjp_principles.route_pop_dailies ON pjp_principles.destinations_additional.route_code = pjp_principles.route_pop_dailies.parent_route
		WHERE pjp_principles.destinations_additional.cust_id = ? 
		  AND pjp_principles.route_pop_dailies.parent_route IS NOT NULL 
		  AND (pjp_principles.destinations_additional.status = 'Approved' OR pjp_principles.destinations_additional.status = 'Approved With Propose')
		  AND pjp_principles.destinations_additional.date BETWEEN ? AND ?
		GROUP BY pjp_principles.destinations_additional.route_code, pjp_principles.routes.route_name
	`
