...
type ValidateOrderRepository interface {
	...
	FindDetailOutletID(OutletID int64) (outlets model.Outlet, err error)
	GetWarehouseStockByProducts(custID string, whID int, proIDs []int64) ([]model.WarehouseStockValidation, error)
}
...
func (repository *RepositoryValidateOrderImpl) FindProductByListID(productIDs []int64) (products []model.Product, err error) {
	err = repository.
		Select("*").
...
func (repository *RepositoryValidateOrderImpl) FindAllArByCustId(dataFilter entity.ValidateOrderBody) ([]model.ArList, int64, int, error) {
	...
	queryCount.Where("sls.order.is_paid_off = false")
	query.Where("sls.order.is_paid_off = false")
...
func (repository *RepositoryValidateOrderImpl) FindDetailOutletID(OutletID int64) (outlets model.Outlet, err error) {
	err = repository.
		Select("*").
...
func (repository *RepositoryValidateOrderImpl) GetWarehouseStockByProducts(custID string, whID int, proIDs []int64) ([]model.WarehouseStockValidation, error) {
	var result []model.WarehouseStockValidation
	err := repository.Table("inv.warehouse_stock whs").
		Select("whs.pro_id, whs.qty, p.conv_unit2, p.conv_unit3").
		Joins("LEFT JOIN mst.m_product p ON p.pro_id = whs.pro_id").
		Where("whs.cust_id = ? AND whs.wh_id = ? AND whs.pro_id IN (?)", custID, whID, proIDs).
		Find(&result).Error
	return result, err
}
