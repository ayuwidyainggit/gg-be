...
func NewValidator() *Validate {
	...
	vc.RegisterValidation("integerTimestamp", integerTimestamp)
	...
	for l := range SupportedLocales {
		...
		case id.New().Locale(): // an inefficiency bc locale names are not available as constants
			...
			vc.RegisterTranslation("integerTimestamp", transId, func(ut ut.Translator) error {
				err := ut.Add("integerTimestampID", "{0} format harus benar dan tahun >= 2024", true) // see universal-translator for details
				if err != nil {
					return err
				}
				return nil
				...
				var t string
				t, _ = ut.T("integerTimestampID", fe.Field())
				return t
			})
		case en.New().Locale(): // an inefficiency bc locale names are not available as constants
			...
			vc.RegisterTranslation("integerTimestamp", transEn, func(ut ut.Translator) error {
				err := ut.Add("integerTimestampEN", "{0} must be valid date and year >= 2024", true) // see universal-translator for details
				if err != nil {
					return err
				}
				return nil
				...
				var t string
				t, _ = ut.T("integerTimestampEN", fe.Field())
				return t
			})
		}
...
func integerTimestamp(fl validator.FieldLevel) bool {
	strIntTimestamp := fl.Field().String()
	i, err := strconv.ParseInt(strIntTimestamp, 10, 64)
	if err != nil {
		return false
	}
	tm := time.Unix(i, 0)
