...
func JWTProtected() func(ctx *fiber.Ctx) error {
	...
	// log.Println("### configPrint ###")
	// log.Println(string(configPrint))
	// log.Println("### End Of configPrint ###")
...
func jwtError(c *fiber.Ctx, err error) error {
	if err.Error() == "Missing or malformed JWT" {
		// custId := c.GetReqHeaders()[constant.CUST_ID][0]
		custId := c.Get(constant.CUST_ID)
...
func jwtSuccess(c *fiber.Ctx) error {
	...
	// claimsPrint, _ := json.Marshal(claims)
